...
class Perfusion_meta():
    ...
    def Tension2(self,Diam,Pres):
        ...
        meta_la = self.SCR(self.xD_la, *cons)
        meta_sa = self.SCR(self.xD_sa, *cons)
        lam_D_la = Diam_la / self.D0_la  # The ratio of diameters...
        lam_D_sa = Diam_sa / self.D0_sa
        Stone_la = (self.Cmyo_la * (Pmid_la * Diam_la * 0.5)) + self.Ctonedd_la - (self.Cshear_la * shear_la) - (self.Cmeta_la * meta_la)  # The stone is the tone of the vessel...
        Stone_sa = (self.Cmyo_sa * (Pmid_sa * Diam_sa * 0.5)) + self.Ctonedd_sa - (self.Cshear_sa * shear_sa) - (self.Cmeta_sa * meta_sa)
    ...
    def Activation(self,Pres, Diam, flag, flag2):
        ...
        if (flag == 1):
            if (flag2 == 1):
                ...
                return (self.Cmyo_la * (Pmid_la * Diam_la * 0.5))
            elif (flag2 == 3):
                return (self.Cshear_la * shear_la)
            elif (flag2 == 4):
                return (self.Cmeta_la * meta_la)
            else:
                return Stone_la
        else:
            if (flag2 == 1):
                ...
                return (self.Cmyo_sa * (Pmid_sa * Diam_sa * 0.5))
            elif (flag2 == 3):
                return (self.Cshear_sa * shear_sa)
            elif (flag2 == 4):
                return (self.Cmeta_sa * meta_sa)
            else:
                return Stone_sa
    ...
    def Calculate(self):
        ...
        while (Pres <= 200):
            ...
            D = fsolve(self.Tension2, D)
            k1, k2 = self.Tension2(D)
        ...
        while (k < len(self.Diameter_la)):
            ...
            Aw1 = np.pi * ((self.n_a * self.l_a) + (self.n_la * self.l_la) + (self.n_sa * self.l_sa) + (self.n_c * self.l_c))
            Aw2 = 2 * np.pi * ((self.n_a * self.l_a * self.Diam_a * 0.5) + (self.n_la * self.l_la * Diam_la * 0.5) + (self.n_sa * self.l_sa * Diam_sa * 0.5) + (self.n_c * self.l_c * self.Diam_c * 0.5))
            Aw3 = np.pi * ((self.n_a * self.l_a * self.Diam_a * self.Diam_a * 0.25) + (self.n_la * self.l_la * Diam_la * Diam_la * 0.25) + (self.n_sa * self.l_sa * Diam_sa * Diam_sa * 0.25) + (self.n_c * self.l_c * self.Diam_c * self.Diam_c * 0.25))
            Aw3 = Aw3 + np.pi * ((self.n_sv * self.l_sv * self.Diam_sv * self.Diam_sv * 0.25) + (self.n_lv * self.l_lv * self.Diam_lv * self.Diam_lv * 0.25) + (self.n_v * self.l_v * self.Diam_v * self.Diam_v * 0.25))
            Aw3 = Aw3 - ((self.n_c * self.l_c) / (500 * 1e06))
            d_t = (-Aw2 + np.sqrt((Aw2 * Aw2) - (4 * Aw1 * Aw3))) / (2 * Aw1)
            d_t = 18.8 * 1e-6
            Diam_a1 = self.Diam_a + (2 * d_t)
            Diam_c1 = self.Diam_c + (2 * d_t)
            Diam_la1 = Diam_la + (2 * d_t)
            Diam_sa1 = Diam_sa + (2 * d_t)
            # Calculate the total volume of the arteries and segments...
            vol_a = np.pi * 0.25 * (Diam_a1 * Diam_a1) * self.l_a * self.n_a
            vol_la = np.pi * 0.25 * (Diam_la1 * Diam_la1) * self.l_la * self.n_la
            vol_sa = np.pi * 0.25 * (Diam_sa1 * Diam_sa1) * self.l_sa * self.n_sa
            vol_c = np.pi * 0.25 * (Diam_c1 * Diam_c1) * self.l_c * self.n_c
            vol_lv = np.pi * 0.25 * (self.Diam_lv * self.Diam_lv) * self.l_lv * self.n_lv
            vol_sv = np.pi * 0.25 * (self.Diam_sv * self.Diam_sv) * self.l_sv * self.n_sv
            vol_v = np.pi * 0.25 * (self.Diam_v * self.Diam_v) * self.l_v * self.n_v
        ...
        # Plot the various activations versus the pressures....
        ...
        Acti_meta_test = []
        k1 = 0
        while (k1 < len(self.Diameter_la)):
            DF = [self.Diameter_la[k1] * 1e-6, self.Diameter_sa[k1] * 1e-6]
            Acti_myo.append(self.Activation(self.Pressure_in[k1], DF, 1, 2))
            Acti_shear.append(self.Activation(self.Pressure_in[k1], DF, 1, 3))
            Acti_meta.append(self.Activation(self.Pressure_in[k1], DF, 1, 4))
            k1 += 1
        # Plot the various activations.....
        return {'Pressure': self.Pressure_in, 'Diameter(LA)': self.Diameter_la, 'Diameter(SA)': self.Diameter_sa,
                'Normalised Perfusion': self.perfusion_norm, 'Activation(Myo)': Acti_myo, 'Activation(shear)': Acti_shear,
                'Activation(meta)': Acti_meta}
    """   
    ...
    plt.xlim(0, 200)
    plt.ylim(0, 20)
    plt.plot(Test_Pressure2, Acti_myo_test, 'b')
    plt.plot(Test_Pressure3, Acti_shear_test, 'r')
    plt.plot(Test_Pressure4, Acti_meta_test, 'g')
    plt.plot(Pressure_in, Acti_myo, 'c')
