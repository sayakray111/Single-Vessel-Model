...
Diam_la = 0.001
Diam_sa = 0.001
Diam_c = float(mat.get('Dcc'))
Diam_c = Diam_c*0.01
Diam_lv = float(mat.get('Dlvc'))
Diam_lv = Diam_lv*0.01
Diam_sv = float(mat.get('Dsvc'))
Diam_sv = Diam_sv*0.01
...
while(Pres<=198.5):
    ...
    D111 = 0
    h = np.exp(Ctoned)
    P22 = (Pvc*133.33)+(Q_tot*(Resistance_v+Resistance_lv+Resistance_sv+Resistance_c))
    P11 = (Pres*133.333)-(Q_tot*Resistance_a)-(Q_tot*Resistance_la)
    P_sa = (P11+P22)*0.5
    D111 = P_sa
    vis_1 = vis_sa
    #X0 = Dekkers(Tension2,0,0.00005,D111)
